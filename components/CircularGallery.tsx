...
class App {
    ...
    createRenderer() {
        ...
        // Initialize Raycast and Mouse here to ensure gl is ready
    ...
    onTouchMove(e: any) {
        ...
        // Calculate movement to detect drag vs click
        const moveDist = Math.abs(x - this.start);
        if (moveDist > 5) {
            this.isClick = false;
        }
    ...
    onTouchUp(e: any) {
        if (this.isClick && this.onItemClick) {
            // Update mouse position for raycast if it's a click (use last known position)
            // For mouse events, e is available. For touch, we might rely on last move or start.
            // But simplify: we need coordinates relative to canvas.
            // Since onTouchUp might not have coordinates, use the Start coordinates if it was a tap.

            // Normalize mouse coordinates using canvas client dimensions
            const gl = this.gl;
